...
site = response.content # tentar alterar depois para
beauty_site = BeautifulSoup(site, 'html.parser')

collecting_data = beauty_site.findAll('a', attrs={'class': 'ui-search-result__content ui-search-link'})

for products in collecting_data:
    price = products.find('span', attrs={'class': 'price-tag-fraction'}).text
    link = products['href']
    product = products.find('h2', attrs={'class': 'ui-search-item__title ui-search-item__group__element'}).text
...
for products in collecting_data:
    ...
    separator = products.find('span', attrs={'class': 'price-tag-decimal-separator'})
    if separator:
